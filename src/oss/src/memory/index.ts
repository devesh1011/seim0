...
export class Memory {
  ...
  /**
   * Add a new memory (fact or conversation) to the store.
   *
   * @param messages - The message(s) or conversation to store.
   * @param config - AddMemoryOptions object. Must include at least one of userId, agentId, or runId.
   *   - userId: string (required for filtering/searching by user)
   *   - agentId: string (optional)
   *   - runId: string (optional, for grouping by workflow/run)
   *   - metadata: Record<string, any> (optional, for custom fields like session, project, etc.)
   *     - Example: { session: "test_fact_extraction", project: "sei_memory_test" }
   *   - filters: SearchFilters (optional, advanced)
   *   - infer: boolean (optional, default true)
   *
   * Note: Only userId, agentId, and runId are treated as first-class filters in search. Other metadata fields (like session) are stored but not directly filterable unless you extend the search logic.
   */
  async add(
  ...
  /**
   * Search for memories matching a query and filters.
   *
   * @param query - The search query string.
   * @param config - SearchMemoryOptions object. Must include at least one of userId, agentId, or runId.
   *   - userId: string (required for filtering/searching by user)
   *   - agentId: string (optional)
   *   - runId: string (optional, for grouping by workflow/run)
   ...
   *   - filters: SearchFilters (optional, advanced)
   *
   * Note: Only userId, agentId, and runId are treated as first-class filters. To search by custom metadata (like session), you must extend the implementation.
   */
  async search(
    query: string,
