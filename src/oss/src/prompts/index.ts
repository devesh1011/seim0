...
/**
 * Memory operation interface for structured LLM responses
 */
...
/**
 * Enhanced fact extraction from mem0 - extracts key facts from conversations
 */
export function getFactRetrievalMessages(
  parsedMessages: string,
): [string, string] {
  const systemPrompt = `You are an expert at extracting key facts and information from conversations. Your task is to identify and extract the most important, specific, and actionable facts that would be valuable to remember.
...
- Extract specific, factual information that is actionable or provides context
- Focus on information that would help in providing personalized responses
- Ignore general knowledge, common facts, or conversational filler
- Each fact should be self-contained and understandable without additional context
- Include relevant context like who, what, when, where if available
- Avoid extracting temporary states unless they indicate patterns
- Prioritize facts that are unique to the individual or situation

Guidelines for What NOT to Extract:
- Avoid general knowledge that is universally known
- Don't extract obvious statements without specific context
- Skip conversational connectors and pleasantries
- Ignore hypothetical or speculative statements unless they reveal preferences
- Don't store temporary emotional states unless they indicate patterns

Format your response as a JSON object with a "facts" array:
{
  "facts": [
    "John works as a software engineer at Google",
    "Sarah prefers coffee over tea and drinks it every morning",
    "The team meeting is scheduled for every Tuesday at 2 PM"
  ]
}

If no meaningful facts can be extracted, return an empty facts array.
...
/**
 * Validates and parses fact extraction response
 */
export function parseFactExtractionResponse(response: string): string[] {
  try {
    const cleaned = removeCodeBlocks(response);
...
/**
 * Validates and parses memory management response
 */
export function parseMemoryOperationsResponse(
  response: string,
): MemoryOperation[] {
  try {
    const cleaned = removeCodeBlocks(response);
    ...
    // Check if it's the expected format with a memory array
      ...
      return parsed.memory
        .filter(
          (op: any) =>
            op &&
            typeof op === "object" &&
            ["ADD", "UPDATE", "DELETE", "NONE"].includes(op.event),
        )
        .map((op: any) => ({
          action: op.event,
          memory_id: op.id,
          memory: op.text,
          reasoning: `${op.event} operation for memory ID ${op.id}`,
          old_memory: op.old_memory,
        }));
    }

    return [];
  } catch (error) {
    console.error("Failed to parse memory operations response:", error);
    return [];
  }
}

/**
 * Generates search prompt for semantic retrieval
 */
export function getSearchPrompt(
  query: string,
  memoryCount: number = 5,
): string {
  return `Based on the user's query, I need to find the most relevant memories from the knowledge base.

Query: "${query}"

Please search for the top ${memoryCount} most relevant memories that could help answer this query or provide useful context.`;
}

/**
 * Generates a prompt for memory summarization
 */
export function getSummarizationPrompt(memories: string[]): string {
  return `Please provide a concise summary of the following memories, highlighting the key facts and relationships:
